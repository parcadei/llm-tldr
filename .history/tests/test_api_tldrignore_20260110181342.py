...
"""
    ...
    def test_scan_project_files_respects_tldrignore(self):
        ...
        file_paths = [Path(f).relative_to(self.project_root) for f in files]
        
        # Should include src files
        assert any("src/main.py" in str(p) for p in file_paths), \
            "src/main.py should be included"
        assert any("src/utils/tools.py" in str(p) for p in file_paths), \
            "src/utils/tools.py should be included"
        
        # Should exclude ignored files
        assert not any("build" in str(p) for p in file_paths), \
