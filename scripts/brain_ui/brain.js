...
function setupGraph() {
    Graph
        ...
        .linkDirectionalParticleColor(() => '#00ff88')  // Bright green
...
function getLinkColor(link) {
    ...
    if (selectedNode) {
        ...
        if (srcId === selectedNode.id) {
            return 'rgba(0, 191, 255, 0.5)';  // DeepSkyBlue tint - outgoing (Bright & Visible)
        } else if (tgtId === selectedNode.id) {
            return 'rgba(255, 68, 68, 0.5)';  // Bright Red tint - incoming
        }
...
function getParticleColor(link) {
    ...
    if (selectedNode) {
        if (srcId === selectedNode.id) return '#00bfff';  // DeepSkyBlue - outgoing (High Contrast)
        if (tgtId === selectedNode.id) return '#ff3333';  // Bright Red - incoming
        return 'rgba(0,255,136,0.15)';  // Faded green for non-connected
    }
    return '#00ff88';  // Bright default green
}
...
function filterByCentrality(threshold) {
    ...
    document.getElementById('centrality-value').innerText = `${(t * 100).toFixed(0)}%`;
    graphData.nodes.forEach(node => {
        node.__visible = (node.centrality || 0) >= t || node.type === 'file';
    });
